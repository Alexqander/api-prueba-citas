/*
  Warnings:

  - A unique constraint covering the columns `[username]` on the table `Citizen` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `username` to the `Citizen` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "Citizen" ADD COLUMN     "username" TEXT NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX "Citizen_username_key" ON "Citizen"("username");
